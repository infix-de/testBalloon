...
fun Project.addTestBalloonPluginFromProject(compilerPluginDependency: Dependency, sharedDependency: Dependency) {
    ...
    afterEvaluate {
        extensions.configure<HasConfigurableKotlinCompilerOptions<*>>("kotlin") {
            val testBalloonExtension = extensions.getByName("testBalloon") as TestBalloonGradleExtension
            compilerOptions {
                compilerPluginOptionValues(testBalloonExtension, testBalloonProperties).forEach { (key, value) ->
                    freeCompilerArgs.addAll("-P", "plugin:de.infix.testBalloon:$key=$value")
                }
            }
        }
    }
...
fun Project.testBalloon(action: TestBalloonGradleExtension.() -> Unit) {
    extensions.configure<TestBalloonGradleExtension>("testBalloon") {
        action()
    }
}
