...
* provides language elements to register and structure tests,
* discovers tests,
* runs tests, and
...
* [x] the dynamic registration of tests via plain Kotlin,
* [x] the dynamic registration of test suites via plain Kotlin.

NOTE: Dynamic registration means that tests or test suites can be registered at run time instead of statically at compile time. It does not refer to registering tests or test suites at test execution time.

==== Rationale

Dynamic registration via plain Kotlin supports arbitrarily parameterized tests, while keeping the API surface small, making it easy to become proficient.
...
* [x] Fixture tear-down occurs automatically when the closing the test suite they were registered in.
