...
. *testBalloon-gradle-plugin*: A Gradle plugin enabling the TestBalloon Kotlin compiler plugin, configuring test tasks to support TestBalloon's test selection, configuring reporting modes, and providing a `testBalloon` extension for configuration.
...
Since ``TestSuite``s nest, they can build a multi-level hierarchy of test elements.
...
The core classes form the test element hierarchy with
...
+
--
NOTE: Top-level test suites wire themselves to the second level of the test element hierarchy, their ``TestCompartment``s, which connect to the `TestSession` at the top.
--
* calls `setUpAndExecuteTests`, which sets up the test element hierarchy and executes tests.

A test execution comprises two phases:

1. `TestSession.global.setUp` sets up the entire test element hierarchy, which includes
* registering lower-level test elements,
* selecting test elements for execution (excluded elements won't appear),
* configuring test elements.
2. `TestSession.global.execute` executes tests (all or a selected subset).
...
1. Open a terminal (the following will probably not work correctly if invoked directly from the IDE).
2. `gradlew -Dorg.gradle.debug=true -Pkotlin.compiler.execution.strategy=in-process :examples:general:clean :examples:general:compileTestKotlinJvm`
--
