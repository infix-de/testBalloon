...
==== Choose Kotlin versions for integration tests
...
. For each minor version, choose a branch under `releases`, like `releases/1.1` (omit the patch version in the branch name).
. Prepare the release branch from `main` or a previous release branch.
. Check out the release branch.
. Change the version in `gradle.properties`, like `version=1.1.0`.
. Adapt versions in `gradle/libs.versions.toml`.
. Make necessary source code changes.
. Run `./release-scripts/00-ALL`
. Verify that everything compiles and runs, accepting failed tests from examples and experiments.
. Commit changes
