...
val IncrementalCompilationTests by testSuite(
    ...
    incrementalCompilationTestSuite("incremental-compilation-testBalloon-jvm") {
        testSeries(
            name = "full compilation",
            testSourceDirectoryName = "test",
            gradleOptions = arrayOf("-Pkotlin.incremental=false"),
            testConfig = TestConfig.disable() // enable to observe behavior with full compilation
        )

        testSeries(name = "incremental compilation", testSourceDirectoryName = "test")
    }
}
...
private class IncrementalCompilationTestProject(private val projectTestSuite: TestSuite, projectName: String) :
    ...
    fun testSeries(
        name: String,
        testSourceDirectoryName: String = "commonTest",
