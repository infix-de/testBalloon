...
open class CompilerPluginBaseTests(val externalEntryPointSourceFile: SourceFile? = null) {
    fun versioning(adapterVersion: String, compilerVersion: String) {
        compilation("", debugLevel = "BASIC") {
            assertContains(
                messages,
                "[DEBUG] using compiler adapter $adapterVersion for Kotlin compiler $compilerVersion"
            )
        }
    }
    ...
    private fun compilation(
        ...
        fun option(name: String, value: String): PluginOption = PluginOption(PLUGIN_ID, name, value)

        try {
            compilation.apply {
                moduleName = "module"
                sources =
                    listOfNotNull(SourceFile.kotlin("Main.kt", sourceCode.trimIndent()), externalEntryPointSourceFile)
                verbose = false
