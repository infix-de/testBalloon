...
class IrGenerationExtensionBase(
    private val compilerConfiguration: CompilerConfiguration,
    private val declarationFinder: (IrPluginContext) -> DeclarationFinderAdapter,
    private val moduleTransformer: (ModuleTransformer.Configuration) -> ModuleTransformer
    ...
    override fun generate(moduleFragment: IrModuleFragment, pluginContext: IrPluginContext) {
        ...
        val configuration: ModuleTransformer.Configuration = try {
            ModuleTransformer.Configuration(
        ...
        moduleFragment.transform(moduleTransformer(configuration), null)
    }
}

@OptIn(UnsafeDuringIrConstructionAPI::class)
abstract class ModuleTransformer(val configuration: Configuration) :
    ...
    class Configuration(
        ...
        override val declarationFinder: DeclarationFinderAdapter,
    ...
    val pluginContext: IrPluginContext get() = configuration.pluginContext

    override val declarationFinder: DeclarationFinderAdapter = configuration.declarationFinder

    override val messageCollector: MessageCollector get() = configuration.messageCollector
    ...
    private fun irTestFrameworkEntryPointProperty(): IrProperty {
        ...
        with(symbol.owner) {
            addAnnotation(symbol, irClassSymbol("kotlin.native.EagerInitialization"))
     ...
     * ```
     * companion object {
     *     @JvmName("testFrameworkDiscoveryResult")
    ...
    private fun irJUnit4RunnerEntryPointClass(entryPointFile: IrFile): IrDeclaration? {
        ...
        return pluginContext.irFactory.buildClass {
            ...
            addFakeOverridesCompatibly(IrTypeSystemContextImpl(irBuiltIns))
    ...
    abstract fun IrClass.addFakeOverridesCompatibly(typeSystem: IrTypeSystemContext)
    ...
    private fun IrProperty.initializeWith(
        ...
        addGetter {
            origin = defaultPropertyAccessor
    ...
    protected fun IrSymbolOwner.irConstructorCall(irClassSymbol: IrClassSymbol, vararg irValues: IrExpression?) =
    ...
    abstract fun IrMutableAnnotationContainer.addAnnotation(
        irTargetSymbol: IrSymbol,
        irAnnotationClassSymbol: IrClassSymbol,
        vararg annotationArguments: IrExpression
    )

    abstract val defaultPropertyAccessor: IrDeclarationOrigin
     ...
     */
    abstract fun registerReference(entryPointFile: IrFile, referencedDeclaration: IrDeclarationWithName)
...
/**
 * Adapter to the `DeclarationFinder` interface introduced in Kotlin 2.3.20.
 */
interface DeclarationFinderAdapter {
...
private interface ModuleWideSymbolResolving : Reporting {
    val declarationFinder: DeclarationFinderAdapter
