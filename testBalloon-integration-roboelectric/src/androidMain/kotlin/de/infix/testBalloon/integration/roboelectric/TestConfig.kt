 ...
 *
 * Settings configured as part of a [roboelectricTestSuite] affect that suite directly.
 * Child elements inherit these settings, unless configured otherwise.
 */
public fun TestConfig.roboelectric(settings: RoboelectricSettings.() -> Unit): TestConfig =
    parameter(RoboelectricContext.Key) { inheritedContext ->
        val inheritedSettings = inheritedContext?.settings
        val settings = RoboelectricSettings(
            portableClasses = inheritedSettings?.portableClasses ?: mutableSetOf(),
            portablePackages = inheritedSettings?.portablePackages ?: mutableSetOf(),
            conscryptMode = inheritedSettings?.conscryptMode,
            applicationLifetime = inheritedSettings?.applicationLifetime
        ).apply { settings() }

        val inheritedConfig = inheritedContext?.config ?: RoboelectricContext.defaultConfig
        val config = Config.Builder(inheritedConfig).overlay(settings.config()).build()

        RoboelectricContext(testElement = this, config = config, settings = settings)
    }

/**
 * Settings for a Roboelectric test suite.
 */
public data class RoboelectricSettings(
    ...
    // region – Attributes inherited by direct initialization from a parent's RoboelectricSettings.

    /**
     * Classes specified to be portable between Roboelectric and the outside JVM.
     *
     * Such classes will be excluded from being instrumented by Roboelectric. The set includes classes inherited
     * from a parent's [RoboelectricSettings]. It may be extended, or cleared and populated anew.
     */
     ...
     *
     * Such packages will be excluded from being instrumented by Roboelectric. The set includes packages inherited
     * from a parent's [RoboelectricSettings]. It may be extended, or cleared and populated anew.
     */
    ...
    /**
     * The lifetime an application (the default is per test).
     */
...
public enum class ApplicationLifetime {
    ...
    /**
     * The application's lifetime is the execution of a Roboelectric test suite.
     *
     * All tests of a Roboelectric test suite execute in a shared application.
     *
     * **Note:** Non-Roboelectric test suites do not control the application's lifetime.
     */
